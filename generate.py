...
def _profile_is_filled(row):
    ...
    return any(row.get(col, '').strip() for col in ('client_type', 'focus_area', 'bio'))


def _build_enquiry_line(submitted):
    """Return the pulsing enquiry line HTML, or empty string if no submitted count."""
    val = str(submitted).strip()
    if not val or val == '0':
        return ''
    return (
        '<div style="border-top:1px solid rgba(255,255,255,.08);padding-top:9px;'
        'display:flex;align-items:center;gap:8px;">'
        '<span style="width:9px;height:9px;border-radius:50%;background:#22c55e;'
        'box-shadow:0 0 10px rgba(34,197,94,.9);flex-shrink:0;'
        'animation:unclaimedPulse 1.8s ease-in-out infinite;"></span>'
        f'<span style="font-family:\'IBM Plex Sans\',sans-serif;font-size:13px;'
        f'font-weight:700;color:#22c55e;letter-spacing:-.01em;">'
        f'{val} new client enquiries this week</span></div>'
    )
